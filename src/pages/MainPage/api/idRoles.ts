...
export const idRoles = create<IFetch>((set, get) => ({ 
    ...
    formateState: () => { 
        ...
        const timeEntrDetail = 
            rolesState && rolesState && Object.entries(rolesState).slice(3, Object.entries(rolesState).length); 
        const timeEntrGeneral = genRolesState && genRolesState && Object.entries(genRolesState); 
        timeEntrDetail && 
            timeEntrDetail.map((el, index) => { 
                if (el[1] === null) { 
                    timeEntrDetail[index][1] = 
                        timeEntrGeneral && timeEntrGeneral[index] ? timeEntrGeneral[index][1] : el[1]; 
                } 
            }); 
